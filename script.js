...
function monitoraNotifiche() {
    db.ref('logs').on('value', s => {
        ...
        chiavi.forEach(key => {
            const l = logs[key];
            if (l.letta && (oraAttuale - l.time > 86400000)) { db.ref('logs/' + key).remove(); return; }
            if (!l.letta) nuovi++;
            const item = document.createElement('div');
            ...
            item.innerHTML = `<div><div style="font-size:10px; color:#888;">${dataOra}</div><div style="font-weight:${l.letta?'normal':'bold'};">${l.msg}</div><div style="font-size:11px; color:var(--p);">${l.dataIt ? '📅 ' + l.dataIt : ''} ${l.ora ? ' 🕒 ' + l.ora : ''}</div></div>`;
            item.onclick = () => {
                db.ref('logs/' + key).update({ letta: true });
                if (l.iso) { toggleVista('g'); selezionaGiorno(l.iso, true); closeModal('notifModal'); }
            };
...
document.getElementById('vMese').addEventListener('touchend', e => { touchendX = e.changedTouches[0].screenX; const p = document.getElementById('monthPicker'); if (touchendX < touchstartX - 70 && p.selectedIndex < p.options.length - 1) { p.selectedIndex++; initCalendar(); } if (touchendX > touchstartX + 70 && p.selectedIndex > 0) { p.selectedIndex--; initCalendar(); } }, false);
