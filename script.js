...
function logAttivita(messaggio, oraEvento = "", isoData = "", idElemento = "") {
    ...
    let dataIt = "";
    if (isoData) { const [y, m, d] = isoData.split("-"); dataIt = `${d}/${m}/${y}`; }
    db.ref('logs').push({ msg: messaggio, ora: oraEvento, dataIt: dataIt, iso: isoData, time: timestamp, letta: false, targetId: idElemento });
}
...
function initCalendar() {
    ...
    for(let d=1; d<=new Date(y, m, 0).getDate(); d++) {
        ...
        db.ref('titoli/'+iso).on('value', s => { 
            ...
            if(el) {
                const val = (s.val() || "").toUpperCase(); el.innerText = val; el.style.display = val ? "block" : "none";
                ...
                if((festName || isDomenica) && el.style.color !== "white") { el.style.backgroundColor = "#d32f2f"; el.style.color = "white"; }
            } 
        });
        db.ref('agenda/'+iso).on('value', s => {
            ...
            Object.values(data).sort((a,b)=> cleanH(a.h)-cleanH(b.h)).forEach(v => {
                ...
                if(testoBreve) {
                    ...
                    const item = document.createElement('div'); item.className = "item-mese"; item.style.backgroundColor = cHex;
...
function selezionaGiorno(data, scroll = false, targetId = "") {
    ...
    const mp = document.getElementById('monthPicker');
    if (mp.value !== `${y}-${m}`) { mp.value = `${y}-${m}`; initCalendar(); }
...
function renderGiorno(targetId = "") {
    ...
    sorted.forEach((item) => {
        if(item.t || mostraTutteRighe || item.isAdmin || item.isBattesimoBlock || item.id.startsWith("ex") || item.id.startsWith("rep_")) {
            ...
            if(item.isBattesimoBlock) {
                ...
                } else if (!item.id.endsWith("_tel") && !item.id.endsWith("_via")) { 
    ...
    if(targetId) { setTimeout(() => { const t = document.getElementById(targetId) || document.getElementById("slot-" + targetId); if(t) { t.scrollIntoView({ behavior: 'smooth', block: 'center' }); t.style.backgroundColor = "#fff9c4"; setTimeout(()=>t.style.backgroundColor="", 2000); } }, 350); }
}
...
function salvaCampo(id, campo, valore, oraDef, isSub=false) { 
    ...
    db.ref(`agenda/${giornoCorrente}/${id}`).update({[campo]:valore, h:oraDef, isSub:isSub});
    if(campo === 't' && valore.length > 3) logAttivita(`Modificato: ${valore.substring(0,30)}`, oraDef, giornoCorrente, id);
}
...
function cambiaColore(id, c, oraDef) { db.ref(`agenda/${giornoCorrente}/${id}`).update({c:(datiGiorno[id]?.c === c ? 'def' : c), h:oraDef}); }
function del(id) { if(confirm("Eliminare?")) { logAttivita(`ELIMINATO: ${datiGiorno[id]?.t || "Impegno"}`, datiGiorno[id]?.h || "", giornoCorrente); db.ref(`agenda/${giornoCorrente}/${id}`).remove(); db.ref(`agenda/${giornoCorrente}/${id}_tel`).remove(); db.ref(`agenda/${giornoCorrente}/${id}_via`).remove(); } }
...
function salvaTitolo(v) { db.ref('titoli/'+giornoCorrente).set(v); logAttivita(`Titolo: ${v}`, "", giornoCorrente, "titoloGiorno"); }
...
function applicaSchemaMatrimonio() {
    ...
    db.ref('titoli/'+giornoCorrente).once('value', s => { let tO = s.val() || ""; db.ref('titoli/'+giornoCorrente).set(tO.includes("MATRIMONIO") ? tO : (tO ? tO + " E MATRIMONIO" : "MATRIMONIO A ")); });
    const sc = [{id:"m"+ts+"_1",t:"SPOSO: ",s:1,tit:"MATRIMONIO"},{id:"m"+ts+"_2",t:"SPOSA: ",s:2},{id:"m"+ts+"_3",t:"CHIESA: ",s:3},{id:"m"+ts+"_4",t:"SALA: ",s:4},{id:"m"+ts+"_5",t:"ESTERNI:",s:5},{id:"m"+ts+"_6",t:"NOTE: ",s:6},{id:"adm_"+ts,isAdmin:true,s:7}];
    sc.forEach(i => { if(i.isAdmin) db.ref(`agenda/${giornoCorrente}/${i.id}`).set({h:"00:00", isAdmin:true, sort:i.s, contratto:false, foto:false, video:false, operatore:""}); else { db.ref(`agenda/${giornoCorrente}/${i.id}`).set({h:(i.s<4?"00:00":""), t:i.t, c:'def', isWed:true, sort:i.s, titolo_mat:i.tit||null}); if(i.t.includes("SPOS")) { db.ref(`agenda/${giornoCorrente}/${i.id}_tel`).set({h:"00:00", t:"TEL: ", c:'def', isSub:true, sort:i.s+0.1}); db.ref(`agenda/${giornoCorrente}/${i.id}_via`).set({h:"00:00", t:"VIA: ", c:'def', isSub:true, sort:i.s+0.2}); } } });
    logAttivita(`Aggiunto Schema MATRIMONIO`, "", giornoCorrente); closeModal('mainModal');
}

function applicaSchemaBattesimo() {
    const id = "bat_" + Date.now(); db.ref('config/'+giornoCorrente).update({mostraOra:false, mostraRighe:false});
    db.ref('titoli/'+giornoCorrente).once('value', s => { let tO = s.val() || ""; db.ref('titoli/'+giornoCorrente).set(tO.includes("BATTESIMO") ? tO : (tO ? tO + " E BATTESIMO" : "BATTESIMO")); });
    db.ref(`agenda/${giornoCorrente}/${id}`).set({ isBattesimoBlock: true, sort: 1, titolo_bat: "BATTESIMO", cerimonia_h: "", cerimonia_t: "", cerimonia_c: "def", ricevimento_h: "", ricevimento_t: "", ricevimento_c: "def", note_t: "", note_c: "def", foto: false, op_foto: "", video: false, op_video: "", acc1: "", dat1: "", chi1: "def" });
    logAttivita(`Aggiunto Schema BATTESIMO`, "", giornoCorrente); closeModal('mainModal');
}

function monitoraNotifiche() {
    db.ref('logs').on('value', s => {
        const logs = s.val() || {}; const cont = document.getElementById('notif-list'); const badge = document.getElementById('notif-badge');
        cont.innerHTML = ""; let nuovi = 0; const ora = Date.now();
        Object.keys(logs).reverse().forEach(k => {
            const l = logs[k]; if (l.letta && (ora - l.time > 86400000)) { db.ref('logs/'+k).remove(); return; }
            if (!l.letta) nuovi++;
            const item = document.createElement('div'); item.className = "notif-item"; item.style.backgroundColor = l.letta ? "transparent" : "#fffde7"; item.style.padding="10px"; item.style.borderBottom="1px solid #eee"; item.style.cursor="pointer";
            item.innerHTML = `<div style="font-size:10px; color:#888;">${new Date(l.time).toLocaleString()}</div><div style="font-weight:${l.letta?'normal':'bold'}">${l.msg}</div><div style="font-size:11px; color:#2196f3;">${l.dataIt ? '📅 '+l.dataIt : ''} ${l.ora ? ' 🕒 '+l.ora : ''}</div>`;
            item.onclick = () => { db.ref('logs/'+k).update({ letta: true }); if (l.iso) { toggleVista('g'); selezionaGiorno(l.iso, true, l.targetId); closeModal('notifModal'); } };
            cont.appendChild(item);
        });
        badge.innerText = nuovi; badge.style.display = nuovi > 0 ? "flex" : "none";
    });
}
...
function condividiWhatsApp() { const tit = document.getElementById('titoloGiorno').value || "Agenda"; let msg = `📅 *${tit}* (${giornoCorrente})\n\n`; if (datiGiorno) { Object.values(datiGiorno).sort((a,b)=>(a.sort||0)-(b.sort||0)).forEach(i => { if(i.isBattesimoBlock) msg += `• *${i.titolo_bat}*\n${i.cerimonia_h? '*'+i.cerimonia_h+'* ':''}${i.cerimonia_t}\n${i.ricevimento_h? '*'+i.ricevimento_h+'* ':''}${i.ricevimento_t}\n`; else if(!i.isSub && !i.isAdmin && i.t && i.t.length > 2) msg += `• ${i.h && i.h !== '00:00' ? '*' + i.h + '* ' : ''}${i.t}\n`; }); } window.open("https://wa.me/?text=" + encodeURIComponent(msg), '_blank'); }
...
window.onload = () => {
    ...
    let isDown = false, startX, scrollLeft, touchstartX;
    // Mouse Drag
    corpo.addEventListener('mousedown', e => { isDown = true; startX = e.pageX - corpo.offsetLeft; scrollLeft = corpo.scrollLeft; });
    corpo.addEventListener('mouseleave', () => isDown = false);
    corpo.addEventListener('mouseup', () => isDown = false);
    corpo.addEventListener('mousemove', e => { if(!isDown) return; e.preventDefault(); const walk = (e.pageX - corpo.offsetLeft - startX) * 2; corpo.scrollLeft = scrollLeft - walk; });
    // Swipe
    corpo.addEventListener('touchstart', e => { touchstartX = e.changedTouches[0].screenX; }, {passive:true});
    corpo.addEventListener('touchend', e => { 
        const tend = e.changedTouches[0].screenX; const p = document.getElementById('monthPicker');
        if (tend < touchstartX - 100 && p.selectedIndex < p.options.length-1) { p.selectedIndex++; initCalendar(); }
        else if (tend > touchstartX + 100 && p.selectedIndex > 0) { p.selectedIndex--; initCalendar(); }
    }, {passive:true});
};
